...
export function apply(ctx, config) {
  ...
  // apply immediately. Services are read with strict=false: long-lived
  // providers (settings-file, webserver) may not satisfy the strict active
  // fiber check even though their service values are set.
  const settingsService = ctx.get("settings", false);
  const scope = settingsService === void 0
    ? void 0
    : settingsService.register(NS, settingsSchema, {
  ...
  if (webServer === void 0) {
    ctx.logger.debug("auto-approval: webServer service unavailable; configuration API disabled");
  } else {
