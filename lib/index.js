...
export function apply(ctx, config) {
  ...
  ctx.inject(["webServer"], (webCtx) => {
    ...
    webCtx.effect(() => webServer.register({ kind: "exact", path: CONFIG_PATH, handler: api }), "dsh-auto-approval: config route");
    webCtx.effect(() => webServer.register({ kind: "exact", path: STATUS_PATH, handler: api }), "dsh-auto-approval: status route");
  });
}
