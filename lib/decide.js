/**
 * dsh-auto-approval-plugin — decision core (pure, dependency-free).
 *
 ...
 * (see rule 4).
 *
 * The set balances convenience against safety: every entry is read-only and
 * side-effect free, and its read surface is no larger than what the DSH
 * `read` tool already allows in any session mode.
 */
export const DEFAULT_HARMLESS_PATTERNS = [
  ...
  "^Get-Command(\\s|$)",
  "^Write-Host(\\s|$)",
  // read-only file/registry inspection (same read surface as the `read` tool)
  "^Get-Content(\\s|$)",
  "^Get-Item(\\s|$)",
  "^Get-ItemProperty(\\s|$)",
  "^Test-Path(\\s|$)",
  "^Resolve-Path(\\s|$)",
  "^Select-String(\\s|$)",
  // read-only machine state probes
  "^tasklist(\\s|$)",
  "^systeminfo(\\s|$)",
  "^ipconfig(\\s|$)",
  "^netstat(\\s|$)",
  ...
  "^Get-NetIPAddress(\\s|$)",
  "^sc\\s+(query|qc|queryex)(\\s|$)",
  // read-only POSIX file inspection
  "^cat(\\s|$)",
  "^head(\\s|$)",
  "^tail(\\s|$)",
  "^wc(\\s|$)",
 ...
 * privilege escalation, persistence, or security-control changes.
 ...
 * New entries favor a slightly wider net over a missed high-impact
 * operation: a false positive merely routes that one request to the human
 * answerer, while a false negative would auto-approve a destructive or
 * privilege-changing command.
 */
export const DEFAULT_DANGEROUS_PATTERNS = [
  ...
  "^[A-Za-z]:[\\\\/]$",
  // account and local-privilege changes
  "\\bnet\\s+(user|localgroup)\\b",
  "\\b(Add|Remove|New|Set|Disable|Enable)-Local(User|Group|GroupMember)\\b",
  "\\b(Add|Remove|Set)-MpPreference\\b",
  "\\bSet-ExecutionPolicy\\b",
  // privilege escalation
  "\\bsudo(\\s|$)|\\bgsudo(\\s|$)|\\b(Start-Process\\s+-Verb\\s+RunAs|runas)(\\s|$)",
  "\\bpsexec\\b|\\bwmic\\s+process\\s+call\\s+create\\b",
  "\\bNew-SelfSignedCertificate\\b",
  // dynamic execution / encoded payloads
  "\\bInvoke-Expression\\b|\\biex\\b|\\-EncodedCommand\\b|\\-enc\\b",
  // registry mutation
  "\\breg\\s+(add|delete|import|save|restore)\\b|\\bregedit\\s+/s\\b",
  // network configuration and firewall changes
  "\\bnetsh\\b|\\b(New|Set|Enable|Disable|Remove)-NetFirewall(Rule|Profile)\\b",
  // service / scheduled-task control
  "\\bsc\\s+(create|delete|config|stop|start|pause|continue)\\b",
  "\\b(Stop|Start|Restart|Set|New|Remove|Disable|Enable)-Service\\b",
  "\\bschtasks\\s+/(create|delete|change|run|end)\\b",
  "\\b(Register|Unregister|Disable|Enable)-ScheduledTask\\b",
  // ACL ownership changes
  "\\btakeown\\b|\\bicacls\\b|\\bcacls\\b|\\bSet-Acl\\b",
  // boot / volume-shadow / secure-delete / event-log tampering
  "\\bvssadmin\\s+delete\\b|\\bbcdedit\\b|\\bcipher\\s+/w\\b",
...
export function classifyRequest({
  ...
  if (COMMAND_TOOLS.has(toolName)) {
    ...
    // Shell metacharacters disqualify this rule too: redirection, pipes and
    // chaining could smuggle an untrusted read or command into the trusted
    // area under the cover of a matching path reference.
    if (workdirTrusted && !SHELL_META.test(command)) {
